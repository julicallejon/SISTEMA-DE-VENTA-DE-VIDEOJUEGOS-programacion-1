...
def mostrar_contenido_archivo():
    """Lee el archivo JSON con json.load() y muestra la informacion almacenada en pantalla."""
    print("\n=== Contenido del archivo " + ARCHIVO_DATOS + " ===")
    try:
        archivo = open(ARCHIVO_DATOS, "r", encoding="utf-8")
        datos = json.load(archivo)
        archivo.close()
        ...
        print("El archivo " + ARCHIVO_DATOS + " no existe todavia.")
...
def agregar_stock(stock, transacciones):
    ...
    transacciones.append({
        ...
        "tipo": "egreso",
        "monto": monto_egreso,
        "descripción": f"Compra de {cantidad}x {producto}",
        "fecha": fecha_egreso
    })
...
def menu_archivos(usuarios, stock, ventas, transacciones):
    """Menu para guardar y leer manualmente el archivo JSON del sistema."""

    continuar = True

    while continuar:
        print(
            "\n=== Menu de archivos (JSON) ===" \
            "\n1. Guardar datos en el archivo JSON" \
            "\n2. Leer y mostrar el contenido del archivo JSON" \
            "\n3. Volver al menu principal"
        )
        ...
        if opcion == "1":
            guardar_datos(usuarios, stock, ventas, transacciones)
            print("\nLos datos fueron guardados correctamente en el archivo JSON.")
...
def menu_principal(stock, ventas, usuarios, transacciones, sesion_activa):
    ...
    while continuar:
        ...
        if n == "1":
            ...
            menu_archivos(usuarios, stock, ventas, transacciones)
