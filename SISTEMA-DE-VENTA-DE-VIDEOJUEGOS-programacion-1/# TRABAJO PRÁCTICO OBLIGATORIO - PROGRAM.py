...
def iniciar_sesion(usuarios, stock, ventas):
    """Permite iniciar sesion con usuario y contraseña. Permite volver al inicio y saltar contraseñas incorrectas."""
    
    isUser = False

    print("\n=== Iniciar Sesion ===")
    print("(Escribir 'volver' si deseas regresar al menú anterior)")
        ...
        for usuario in usuarios:
            if usuario[1] == nombre:
                if usuario[3] == clave:
                    ...
                    print("\nContraseña incorrecta. Permitiendo el ingreso igualmente")
...
def registrar_usuario(usuarios):
    ...
    if re.match(r"^[\w\.]+@[\w\.]+\.[a-z]{2,}$", email):
        email_existente = list(filter(lambda usuario: usuario["email"] == email, usuarios))  
